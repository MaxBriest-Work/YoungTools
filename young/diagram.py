...
class YoungDiagram(object):
    ...
    def __mul__(self,other:"YoungDiagram") -> "YoungDiagram" :
        assert isinstance(other,YoungDiagram), "The input for `other` needs to be a Young diagram."
        return YoungDiagram.Construct_From_Binary_Description(self._binary_description+other._binary_description)
    ...
    def __init__(self, height:int, width:int, usual_description:tuple[int] ) -> None :
        ...
        # Usual description
        assert isinstance(usual_description, tuple) , "The input for `usual_description` needs to be a tuple."
        assert len(usual_description) <= self._height , "The length of `usual_description` (number of rows) needs to be less than or equal to the height {h}.".format(h=self._height)
        stock = []
        previous_row_length = self._width
        for row_counter, row_length in enumerate(usual_description) :
            assert row_length in ZZ, "The {i}-th entry in `usual_description` needs to be an integer.".format(i=row_counter)
            assert 0 <= row_length, "The {i}-th entry in `usual_description` needs to be non-negative.".format(i=row_counter)
            assert row_length <= previous_row_length, "The {i}-th row length needs less than or equal to the previous one.".format(i=row_counter)
            stock += [ row_length ]
    ...
    def __lt__(self, other: "Bundle") -> bool:
        ...
        """
        assert isinstance(other, YoungDiagram), "The input for `other` needs to be a Young diagram."
        assert self._height == other._height, "The two Young diagrams have different height."
        assert self._width == other._width, "The two Young diagrams have different width."
    ...
    def Construct_From_Binary_Description(string:str) -> "YoungDiagram" :
        ...
        assert n == height+width, "The binary description contains a symbol different from `U` or `R`."
        column_heights = []
        ...
        row_widths = [ sum([ 1 for column_height in column_heights if row_counter <= column_height-1 ]) for row_counter in range(height) ]
        usual_description = tuple(row_widths)
        return YoungDiagram(height=height,width=width,usual_description=usual_description)

    @staticmethod
    def Construct_From_Usual_Description(*mu:tuple[int]) -> "YoungDiagram" :
        height = len(mu)
        for counter, entry in enumerate(mu) :
            assert entry in ZZ, "The {i}-th entry needs to be an integer.".format(i=counter)
        width  = max(mu)
    ...
    def is_upper_triangular(self) -> bool :
        if 0 < self._height :
            stock = [ row_length <= self._width*(self._height-row_counter)/self._height
    ...
    def show(self) -> "Table" :
        if 0 < self._height and 0 < self._width : 
            return table( rows=[ row_length*['x']+(self._width-row_length)*[' '] for row_length in self._usual_description ], frame=True )
        else :
            return None
    ...
    def transpose(self) -> "YoungDiagram" :
        return YoungDiagram(height=self._width,width=self._height,usual_description=self.column_lengths())
