            ...
            if not YD_did_already_appear :
                ...
                for fellow in YD.cyclic_orbit() :
                    if fellow.is_upper_triangular() :
                        cyclic_orbit += [ fellow ]
                        if fellow < minimal_object : minimal_object = fellow
                cyclic_orbits += [ cyclic_orbit ]
                yield minimal_object

    def get_upper_triangulars (self) -> Iterator[ YoungDiagram ] :
        if 0 < self._height :
            ceiling = [ floor( self._width*(self._height-row_counter-1)/self._height )
                        for row_counter in range(self._height)
                      ]
            usual_descriptions = [ tuple(usual_description)
                                   for usual_description in IntegerListsLex( max_part=self._width , length=self._height , ceiling=ceiling , max_slope=0 )
                                 ]
            usual_descriptions.reverse()
            return ( YoungDiagram( self._width , self._height , usual_description )
